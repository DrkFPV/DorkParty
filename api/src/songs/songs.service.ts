...
export class SongsService {
    ...
    async addSong(addSongDto:AddSongDto, userId:string): Promise<PlaylistDocument | undefined> {
        const {playlistId , ...song} = addSongDto
        const playlist = await this.playlistModel.findOneAndUpdate(
            { _id:addSongDto.playlistId, userId:userId },
            { $addToSet: { 'songs': song }},
    ...
    async removeSong(removeSongDto:RemoveSongDto, userId:string): Promise<PlaylistDocument | undefined>{
        const {playlistId , ...song} = removeSongDto;
        var playlist = await this.playlistModel.findOneAndUpdate(
            { _id:removeSongDto.playlistId, userId:userId },
